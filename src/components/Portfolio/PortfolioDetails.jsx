...
const PortfolioDetails = () => {
  ...
  return (
    ...
    <p  className='close-port'><AiOutlineClose className='close-port1' onClick={()=>navigate('/portfolio')}/></p>
        <div className='section-details'>
          <div className="carousel">
            <div className="left-carousel">
              <span onClick={prevSlide}><FiArrowLeft/></span>
              </div>
            {filtered.allimage && filtered.allimage.map((img, idx) => (
              <img
                key={idx}
                className={currentIndex === idx ? "image-carousel" : "image-carousel img_hiddenCarousel"
                }
                src={img}
                alt=""
              />
            ...
            <div className="right-carousel">
              <span onClick={nextSlide}><FiArrowRight/></span>
              </div>
          </div>
          <div className="portfolio-content">
            ...
            <p className='techstack'>{filtered.techStack.map((skill,idx) => (
              <span className='techstack1' key={idx}>{ skill}</span>
            ))}</p> 
            <div className='live-preview'></div>
              <a className='a-link' href={filtered['project-link']} target='blank'>Live Preview</a>
